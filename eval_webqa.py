...
class Args:
    ...
    audio_dir: str = field(
        default="your_audip_directory_path",
        metadata={'help': 'Where the images located on.'}
    )
    encoder: str = field(
        default="BAAI/bge-m3",
...
def index1(model: Flag_bgev_model, corpus: [datasets.Dataset, datasets.Dataset], batch_size: int = 256, max_length: int=512, index_factory: str = "Flat", save_path: str = None, save_embedding: bool = False, load_embedding: bool = False):
    """
    1. Encode the entire corpus into dense embeddings; 
    2. Create faiss index; 
    3. Optionally save embeddings.
    ...
    if load_embedding:
        ...
        audio_corpus_embeddings = model.encode_corpus(audio_corpus, batch_size=batch_size, max_length=max_length, corpus_type='audio')

        
        corpus_embeddings = audio_corpus_embeddings
    ...
    # create faiss index
    faiss_index_audio = faiss.index_factory(dim, index_factory, faiss.METRIC_INNER_PRODUCT)
    ...
    if model.device == torch.device("cuda"):
        ...
        faiss_index_audio = faiss.index_cpu_to_all_gpus(faiss_index_audio, co)
    ...
    audio_corpus_embeddings = audio_corpus_embeddings.astype(np.float32)
    faiss_index_audio.train(audio_corpus_embeddings)
    faiss_index_audio.add(audio_corpus_embeddings)

    return faiss_index_audio

def index2(model: Flag_bgev_model, corpus: [datasets.Dataset, datasets.Dataset], batch_size: int = 256, max_length: int=512, index_factory: str = "Flat", save_path: str = None, save_embedding: bool = False, load_embedding: bool = False):
    """
    1. Encode the entire corpus into dense embeddings; 
    2. Create faiss index; 
    3. Optionally save embeddings.
    ...
    if load_embedding:
        ...
        caption_corpus_embeddings = model.encode_corpus(caption_corpus, batch_size=batch_size, max_length=max_length, corpus_type='text')
        print(151,caption_corpus_embeddings.shape)
        
        corpus_embeddings = caption_corpus_embeddings
    ...
    # create faiss index
    faiss_index_caption = faiss.index_factory(dim, index_factory, faiss.METRIC_INNER_PRODUCT)
    ...
    if model.device == torch.device("cuda"):
        ...
        faiss_index_caption = faiss.index_cpu_to_all_gpus(faiss_index_caption, co)
    ...
    caption_corpus_embeddings = caption_corpus_embeddings.astype(np.float32)
    faiss_index_caption.train(caption_corpus_embeddings)
    faiss_index_caption.add(caption_corpus_embeddings)

    return faiss_index_caption
...
def search2(model: Flag_bgev_model, queries: datasets, faiss_index: faiss.Index, k:int = 100, batch_size: int = 256, max_length: int=512):
    """
    1. Encode queries into dense embeddings;
    2. Search through faiss index
    """
    query_embeddings = model.encode_queries(queries["q_audio"], batch_size=batch_size, max_length=max_length, query_type="audio")
    query_size = len(query_embeddings)
    print(152,query_size)
    print(153,query_embeddings.shape)
...
def evaluate(preds, labels, cutoffs=[1,5,10,20,50,100]):
    """
    Evaluate MRR and Recall at cutoffs.                     #这里输入的preds和label是嵌套列表,列表中的每个元素都是列表,都对应一个样本
    """                                                     
    metrics = {}
    
    # MRR
    mrrs = np.zeros(len(cutoffs))
    for pred, label in zip(preds, labels):                          #这里得到的pred是一个包含多个content的列表，label是positive那个列表
        jump = False
...
def evaluate1(preds, labels, cutoffs=[1,5,10,20,50,100]):
    """
    ...
    for pred, label in zip(preds, labels):
        jump = False
        for i, x in enumerate(pred, 1):
            if any(item in label for item in x):
                for k, cutoff in enumerate(cutoffs):
                    if i <= cutoff:
                        mrrs[k] += 1 / i
                jump = True
    ...
    for pred, label in zip(preds, labels):
        if not isinstance(label, list):
            label = [label]
        for k, cutoff in enumerate(cutoffs):
            recall = np.concatenate([np.intersect1d(label, sub_pred) for sub_pred in pred[:cutoff]])
...
def main():
    ...
    df = pd.read_json("/home/ls/FlagEmbedding/research/visual_bge/visual_bge/BGE-m3/caption_query_data1.jsonl", lines=True)
    ...
    df_audio = pd.read_json("/home/ls/FlagEmbedding/research/visual_bge/visual_bge/BGE-m3/audio_corpus_data1.jsonl", lines=True)
    audio_corpus = Dataset.from_pandas(df_audio)
    ...
    dataset_dict = datasets.DatasetDict({"audio": audio_corpus})
    all_corpus = dataset_dict["audio"]
    ...
    metrics = evaluate1(retrieval_results, ground_truths)
...
#     eval_data = datasets.load_dataset('json', data_files="/home/ls/FlagEmbedding/research/visual_bge/visual_bge/BGE-base/audio_query_data2.jsonl", split='train')
#     # mm_it_corpus = datasets.load_dataset('json',  data_files="the_path_to/mm_it_corpus.jsonl", split='train')
#     # text_corpus = datasets.load_dataset('json', data_files="the_path_to/text_corpus.jsonl", split='train')
#     caption_corpus = datasets.load_dataset('json', data_files="/home/ls/FlagEmbedding/research/visual_bge/visual_bge/BGE-base/caption_corpus_data2.jsonl", split='train')
#     model = Flag_bgev_model(model_name_bge = "BAAI/bge-m3",
#                         # model_name_eva = "EVA02-CLIP-B-16", # "EVA02-CLIP-B-16",
#                         normlized = True,
#                         eva_pretrained_path = "eva_clip",
...
#     #     load_embedding=args.load_embedding
#     # )
#     faiss_index_caption= index2(
#         model=model, 
#         corpus=caption_corpus, 
#         batch_size=args.batch_size,
#         max_length=args.max_passage_length,
#         index_factory=args.index_factory,
#         save_path=args.save_path,
...
#     # 将 GPU 索引转换为 CPU 索引
#     cpu_index = faiss.index_gpu_to_cpu(faiss_index_caption)

#     # 然后可以将 CPU 索引保存到磁盘
#     faiss.write_index(cpu_index, "./problem/faiss_index_caption.index")

#     scores, indices = search2(
#         model=model, 
#         queries=eval_data, 
#         faiss_index=faiss_index_caption, 
#         k=args.k, 
#         batch_size=args.batch_size, 
#         max_length=args.max_query_length
#     )
#     with open("./problem/indices.txt", "w") as f:
#         for row in indices:
#             f.write(" ".join(map(str, row)) + "\n")

#     print(154,len(indices))
#     print(155,len(indices[0]))
#     print(156,len(indices[1]))
#     # # 获取并打印 indices 中的前100项及每个项的前几个元素
#     # for index in indices[:100]:
#     #     print(index[:40])  # 假设你只关心每个子列表的前10个元素
...
#     # dataset_dict = datasets.DatasetDict({"text": text_corpus, "mm_it": mm_it_corpus})
#     # all_corpus = datasets.concatenate_datasets([dataset_dict["text"], dataset_dict["mm_it"]])
#     dataset_dict = datasets.DatasetDict({"caption": caption_corpus})
#     all_corpus = dataset_dict["caption"]
...
#     metrics = evaluate(retrieval_results, ground_truths)
#     with open("./problem/retrieval_output.jsonl", "w", encoding="utf-8") as f:
#         for retrieved, truth in zip(retrieval_results, ground_truths):
#             json.dump({"retrieved": retrieved, "ground_truth": truth}, f, ensure_ascii=False)
#             f.write("\n")
